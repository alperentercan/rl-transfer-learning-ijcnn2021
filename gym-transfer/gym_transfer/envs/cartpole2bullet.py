"A modified version of cartpole env using pybullet with collisions"
...
class CartPoleBulletEnv(gym.Env):
    ...
    def __init__(self):
        ...
        if self.TRIG_OBS: #[Cart Position, Cart Velocity, Pole Angle Sine, Pole Angle Cosine, Pole Vel.]
            high = np.array([
                self.x_threshold * 2,
...
#         angle = str("%.2f" % round(180*self.TOP_TOL/math.pi,2))
        ...
        return ("CartPole : " + "INIT_BOTT, "*self.INIT_BOTT + "TRIG_OBS, "*self.TRIG_OBS + (f"Reward: {self.CONS_ROUNDS} rounds, {angle} tol. CONSEC")*self.REW_TOP_CONS + (f"Reward: {angle} tol. HIT_TOP")*self.REW_TOP_TOL + (f"x_threshold = {self.x_threshold}")*(self.x_threshold!= 2.4) + "_Markovian"*self.MAKE_MARKOVIAN)